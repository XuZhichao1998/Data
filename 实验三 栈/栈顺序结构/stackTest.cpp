...
Bool postfix( Stack &S, char *str, double &v )
{
	...
	p = str;
	while ( *p ) {
		if (isOpand(*p)) Push( S, (double)(*p-'0') ); //是数字 
		else if (isOperator(*p))  //是  + - * / 
		{
...
void ExplanComm_Sq( Stack &S )
{
	...
	show();
	do {
		cout <<  "input comm(输入命令): ";
		fflush(stdin);
		cin >> key;
		switch( key ) {
			case '+' : 
				cin >> data;		
				Comm_ADD( S, data );  //push() 
				break;
			case '-' : 
				Comm_SUB( S, data ); //pop() 
				break;
